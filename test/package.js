...
describe('Package', () => {
  describe('#load', () => {
    ...
    it('remote_dereference', async () => {
      const descriptor =
        'https://raw.githubusercontent.com/frictionlessdata/datapackage-js/master/data/data-package-dereference.json'
      const dataPackage = await Package.load(descriptor)
      assert.deepEqual(
  ...
  describe('#descriptor (dereference)', () => {
    ...
    it('pointer bad', async () => {
      const descriptor = {
        resources: [{ name: 'name1', data: ['data'], schema: '#/schemas/main' }],
      }
      const error = await catchError(Package.load, descriptor)
      assert.instanceOf(error, Error)
    ...
    it('remote bad', async () => {
      const descriptor = {
        resources: [{ name: 'name1', data: ['data'], schema: 'http://example.com/schema' }],
      }
    ...
    it('local bad', async () => {
      const descriptor = {
        resources: [{ name: 'name1', data: ['data'], schema: 'bad-path.json' }],
      }
      const error = await catchError(Package.load, descriptor, { basePath: 'data' })
      assert.instanceOf(error, Error)
      if (process.env.USER_ENV !== 'browser') {
    ...
    it('local bad not safe', async () => {
      const descriptor = {
        resources: [{ name: 'name1', data: ['data'], schema: '../data/table-schema.json' }],
      }
      const error = await catchError(Package.load, descriptor, { basePath: 'data' })
      assert.instanceOf(error, Error)
      if (process.env.USER_ENV !== 'browser') {
  ...
  describe('#save', () => {
    ...
    it.skip('general', async function () {
      ...
      writeFile.restore()
      sinon.assert.calledWith(writeFile, 'target', JSON.stringify(expand(descriptor)))
    })
  })
      ...
      resources: [
        {
          ...
          schema: {
            fields: [{ name: 'id' }, { name: 'name' }, { name: 'surname' }, { name: 'parent_id' }],
            foreignKeys: [
              {
                fields: 'name',
